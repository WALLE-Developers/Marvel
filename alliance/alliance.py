  ...
  async def timezone(self, ctx, timezone: str):
    ...
    try:
      if "none" in timezone:
        ...
        await ctx.author.edit(nick=f"[GMT {timezone}] {ctx.author.name}")
        await ctx.send(f"Done! Your timezone has been added to your nickname as `[GMT {timezone}] {ctx.author.name}`.")
